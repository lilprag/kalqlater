...
export default async function PersonalityPage({ params }) {
  ...
  return <><JsonLd data={jsonLd} /><article className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
    ...
    <div className="mt-10 flex flex-wrap gap-3"><a href={productionAppUrl('/test')} className="button-primary">{labels.test}<span aria-hidden="true">→</span></a><Link href={compareHref} className="button-secondary">{labels.compare}</Link><Link href={localePath(locale, `personality/${profile.slug}/careers`)} className="button-secondary">{labels.careers} {code}</Link></div>
  </article></>;
}
