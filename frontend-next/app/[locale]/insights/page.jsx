...
export default async function InsightsHubPage({ params }) {
  ...
  return <><JsonLd data={jsonLd} /><main className="overflow-hidden">
    ...
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8"><div className="grid gap-7 lg:grid-cols-4"><article className="rounded-[2rem] bg-brand-ink p-7 text-white shadow-[0_24px_70px_rgba(45,40,37,.16)] sm:p-10"><p className="section-kicker text-brand-sand">{c.live}</p><h2 className="display-font mt-3 text-4xl">{c.liveTitle}</h2><p className="mt-5 max-w-2xl leading-relaxed text-white/80">{c.liveBody}</p><Link href={localePath(locale, 'insights/communication')} className="button-light mt-7">{c.explore}<span aria-hidden="true">→</span></Link></article><article className="rounded-[2rem] border border-brand-line bg-white p-7 sm:p-10"><p className="section-kicker">{c.live}</p><h2 className="display-font mt-3 text-3xl text-brand-ink">{c.conflictTitle}</h2><p className="mt-4 leading-relaxed text-brand-subtle">{c.conflictBody}</p><Link href={localePath(locale, 'insights/conflict')} className="button-secondary mt-7">{c.conflictExplore}<span aria-hidden="true">→</span></Link></article><article className="rounded-[2rem] border border-brand-line bg-white p-7 sm:p-10"><p className="section-kicker">{c.live}</p><h2 className="display-font mt-3 text-3xl text-brand-ink">{c.leadershipTitle}</h2><p className="mt-4 leading-relaxed text-brand-subtle">{c.leadershipBody}</p><Link href={localePath(locale, 'insights/leadership')} className="button-secondary mt-7">{c.leadershipExplore}<span aria-hidden="true">→</span></Link></article><article className="rounded-[2rem] border border-brand-line bg-white p-7 sm:p-10"><p className="section-kicker">{c.live}</p><h2 className="display-font mt-3 text-3xl text-brand-ink">{c.learningTitle}</h2><p className="mt-4 leading-relaxed text-brand-subtle">{c.learningBody}</p><Link href={localePath(locale, 'insights/learning')} className="button-secondary mt-7">{c.learningExplore}<span aria-hidden="true">→</span></Link></article></div></section>
