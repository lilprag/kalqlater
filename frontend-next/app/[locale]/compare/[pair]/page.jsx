...
export default async function ComparisonPage({ params }) {
  ...
  if (content.version === 'v3') return <EnglishV3Comparison jsonLd={jsonLd} content={content} first={first} second={second} parsed={parsed} related={related} locale={locale} />;
...
function EnglishV3Comparison({ jsonLd, content, first, second, parsed, related, locale }) {
  return <><JsonLd data={jsonLd} /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8"><nav aria-label="Breadcrumb" className="text-sm text-brand-subtle"><Link href={localePath(locale)}>KalQLater</Link><span aria-hidden="true"> / </span><Link href={localePath(locale, 'compare')}>Compare</Link><span aria-hidden="true"> / </span><span>{first.code} comparisons</span><span aria-hidden="true"> / </span>{first.code} and {second.code}</nav>
    <header className="relative mt-5 overflow-hidden rounded-[2rem] bg-brand-ink p-7 text-white shadow-[0_24px_70px_rgba(45,40,37,.2)] sm:p-10 lg:p-12"><span aria-hidden="true" className="absolute -right-12 -top-16 h-64 w-64 rounded-full bg-brand-teal/35 blur-3xl" /><div className="relative"><p className="section-kicker text-brand-sand">{content.eyebrow}</p><h1 className="display-font mt-4 text-4xl leading-tight sm:text-6xl">{content.title || `${first.code} vs ${second.code}: Interaction Intelligence`}</h1><p className="mt-4 max-w-4xl text-lg leading-relaxed text-white/85">{content.helper}</p><p className="mt-4 max-w-4xl text-sm leading-relaxed text-white/70">{content.intro}</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><TypePanel profile={first} align="left" /><TypePanel profile={second} align="right" /></div></div></header>
    <section className="mt-10 rounded-[1.75rem] border border-brand-teal/25 bg-brand-teal/5 p-6 sm:p-8"><p className="section-kicker">Quick answer</p><h2 className="display-font mt-2 text-3xl text-brand-ink">How are {first.code} and {second.code} different?</h2><p className="mt-4 max-w-5xl leading-relaxed text-brand-subtle">{content.quickAnswer}</p><p className="mt-5 rounded-xl bg-white px-4 py-3 text-sm text-brand-subtle"><strong className="text-brand-ink">No compatibility score.</strong> {content.scoreNote.replace(/^No compatibility score:\s*/,'')}</p></section>
    <section className="mt-10"><Header eyebrow="At a glance" title={`${first.code} and ${second.code} comparison`} /><div className="mt-5 overflow-hidden rounded-[1.5rem] border border-brand-line bg-white"><div className="hidden grid-cols-[.8fr_1fr_1fr] bg-brand-ink px-5 py-3 text-sm font-semibold text-white sm:grid"><span>Dimension</span><span>{first.code}</span><span>{second.code}</span></div>{content.atGlance.map(([dimension,a,b])=><article key={dimension} className="grid gap-2 border-t border-brand-line p-4 first:border-t-0 sm:grid-cols-[.8fr_1fr_1fr]"><h3 className="font-semibold text-brand-ink">{dimension}</h3><p className="text-sm text-brand-subtle"><span className="mr-2 font-semibold text-brand-teal sm:hidden">{first.code}:</span>{a}</p><p className="text-sm text-brand-subtle"><span className="mr-2 font-semibold text-brand-plum sm:hidden">{second.code}:</span>{b}</p></article>)}</div></section>
    <section className="mt-10 grid gap-4 lg:grid-cols-2"><V3Card title="Key differences" body={content.keyDifferences}/><V3Card title="What they have in common" body={content.commonGround}/><V3Card title="Communication" body={content.communication}/><V3Card title="Decision-making" body={content.decisionMaking}/><V3Card title="Conflict and repair" body={`${content.conflict.pattern} Escalation risk: ${content.conflict.escalation} Repair: ${content.conflict.repair}`}/><V3Card title="Under stress" body={content.stress}/><V3Card title="Friendship" body={content.friendship}/><V3Card title="Romantic relationships" body={content.romance}/><V3Card title="Work and coworker dynamics" body={content.work}/><V3Card title="Emotional and interpersonal needs" body={content.emotionalNeeds}/></section>
    <section className="mt-10 rounded-[1.75rem] bg-brand-ink p-6 text-white sm:p-8"><p className="section-kicker text-brand-sand">Interaction intelligence</p><h2 className="display-font mt-2 text-3xl">How this pairing actually works</h2><ol className="mt-5 grid gap-3 lg:grid-cols-4">{content.interactionLoop.map((step,index)=><li key={step} className="rounded-xl bg-white/10 p-4 text-sm leading-relaxed text-white/85"><span className="mb-2 block font-bold text-brand-sand">0{index+1}</span>{step}</li>)}</ol></section>
    <section className="mt-10"><Header eyebrow="Translation, not assumption" title="Common misunderstandings" /><div className="mt-5 grid gap-4 lg:grid-cols-3">{content.misunderstandings.map((item,index)=><article key={`${item.from}-${index}`} className="rounded-2xl border border-brand-line bg-white p-5"><p className="text-xs font-bold uppercase tracking-wide text-brand-teal">From {item.from}</p><h3 className="mt-2 font-semibold text-brand-ink">Possible signal</h3><p className="mt-1 text-sm text-brand-subtle">{item.signal}</p><h3 className="mt-4 font-semibold text-brand-ink">What may be heard</h3><p className="mt-1 text-sm text-brand-subtle">{item.heard}</p><p className="mt-4 rounded-xl bg-brand-cream p-3 text-sm text-brand-ink"><strong>Another possibility:</strong> {item.possibility}</p></article>)}</div></section>
    <section className="mt-10 rounded-[1.75rem] bg-brand-sand/25 p-6 sm:p-8"><Header eyebrow="In context" title="Real-life interaction scenarios" /><div className="mt-5 grid gap-4 lg:grid-cols-2">{content.scenarios.map((item)=><article key={item.situation} className="rounded-2xl bg-white p-5"><h3 className="font-semibold text-brand-ink">{item.situation}</h3><dl className="mt-3 space-y-2 text-sm text-brand-subtle"><div><dt className="inline font-semibold text-brand-teal">{first.code}: </dt><dd className="inline">{item.typeA}</dd></div><div><dt className="inline font-semibold text-brand-plum">{second.code}: </dt><dd className="inline">{item.typeB}</dd></div><div><dt className="inline font-semibold text-brand-ink">Possible friction: </dt><dd className="inline">{item.friction}</dd></div><div><dt className="inline font-semibold text-brand-ink">What helps: </dt><dd className="inline">{item.helps}</dd></div></dl></article>)}</div></section>
    <section className="mt-10 grid gap-4 lg:grid-cols-2"><V3List title={`If you are ${first.code} dealing with ${second.code}`} items={content.adviceForTypeA}/><V3List title={`If you are ${second.code} dealing with ${first.code}`} items={content.adviceForTypeB}/></section>
    <section className="mt-10 rounded-[1.75rem] border border-brand-line bg-white p-6 sm:p-8"><Header eyebrow="Growth" title="What each can learn from the other" body={content.learnFromEachOther}/></section>
    <section className="mt-10"><Header eyebrow="Pair-specific myths" title="Myth and reality" /><div className="mt-5 grid gap-4 lg:grid-cols-2">{content.myths.map(([myth,reality])=><article key={myth} className="rounded-2xl bg-brand-cream p-5"><p className="text-xs font-bold uppercase tracking-wide text-brand-plum">Myth</p><h3 className="mt-2 font-semibold text-brand-ink">{myth}</h3><p className="mt-4 text-xs font-bold uppercase tracking-wide text-brand-teal">Reality</p><p className="mt-2 text-sm leading-relaxed text-brand-subtle">{reality}</p></article>)}</div></section>
    <section className="mt-10"><Header eyebrow="Search-driven questions" title="Frequently asked questions" /><div className="mt-5 space-y-3">{content.faq.map(([question,answer])=><details key={question} className="rounded-2xl border border-brand-line bg-white p-5"><summary className="cursor-pointer font-semibold text-brand-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal">{question}</summary><p className="mt-4 max-w-5xl leading-relaxed text-brand-subtle">{answer}</p></details>)}</div></section>
    <section className="mt-10 rounded-[1.75rem] bg-brand-sand/25 p-6 sm:p-8"><Header eyebrow="Continue exploring" title="Related personality guidance" /><div className="mt-5 flex flex-wrap gap-3"><Link href={getPersonalityUrl(first.code,locale)} className="button-secondary">{first.code} personality guide</Link><Link href={getPersonalityUrl(second.code,locale)} className="button-secondary">{second.code} personality guide</Link><Link href={localePath(locale,`personality/${first.slug}/careers`)} className="button-secondary">{first.code} career guide</Link><Link href={localePath(locale,`personality/${second.slug}/careers`)} className="button-secondary">{second.code} career guide</Link><Link href={localePath(locale,'insights/communication')} className="button-secondary">Communication Insights</Link></div><div className="mt-6 flex flex-wrap gap-2">{related.map(item=><Link key={item.slug} href={localePath(locale,`compare/${item.slug}`)} className="button-secondary">{item.first} and {item.second}</Link>)}</div></section>
    <ComparisonPairSilos locale={locale} first={first.code} second={second.code} currentSlug={parsed.slug}/><RelatedContent sourceEntityId={`compare:${parsed.slug}`} sourceType="compare" locale={locale} mode="public" />
    <section className="mt-10 rounded-[1.75rem] bg-brand-teal p-7 text-white"><h2 className="display-font text-3xl">Turn a type difference into a better next conversation.</h2><p className="mt-3 max-w-3xl text-white/80">Use this guide as a prompt for observation and choice—not a verdict on a relationship, ability, or future.</p><div className="mt-6 flex flex-wrap gap-3"><Link href={localePath(locale,'test')} className="button-light">Take the personality test</Link><Link href={localePath(locale,'compare')} className="button-dark-outline">Explore another comparison</Link></div></section>
  </main></>;
}

function V3Card({title,body}) { return <article className="rounded-[1.5rem] border border-brand-line bg-white p-5 sm:p-6"><h2 className="display-font text-2xl text-brand-ink">{title}</h2><p className="mt-3 text-sm leading-relaxed text-brand-subtle">{body}</p></article>; }
function V3List({title,items}) { return <article className="rounded-[1.5rem] border border-brand-line bg-white p-5 sm:p-6"><h2 className="display-font text-2xl text-brand-ink">{title}</h2><ul className="mt-4 space-y-2">{items.map(item=><li key={item} className="rounded-xl bg-brand-cream px-4 py-3 text-sm text-brand-subtle">{item}</li>)}</ul></article>; }
