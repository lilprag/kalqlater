...
export default async function ComparisonPage({ params }) {
  ...
  return <><JsonLd data={jsonLd} /><article className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
    ...
    <div className="mt-10 flex flex-wrap gap-3"><Link href={getPersonalityUrl(first.code, locale)} className="button-secondary">{first.code}</Link><Link href={getPersonalityUrl(second.code, locale)} className="button-secondary">{second.code}</Link><a href={productionAppUrl('/test')} className="button-primary">{labels.test}<span aria-hidden="true">→</span></a></div>
  </article></>;
}
