...
export async function GenericLocaleChrome({ locale, children }) {
  ...
  if (!chrome) notFound();
  return <><header className="sticky top-0 z-40 border-b border-brand-line bg-white/90 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center gap-5 px-4 py-3 sm:px-6"><Link href={localePath(locale)} className="display-font text-xl font-bold text-brand-ink">KalQLater</Link><nav className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-brand-subtle" aria-label="Primary navigation">{NAVIGATION_KEYS.filter((key) => chrome.navigation[key]).map((key) => <Link key={key} href={destination(locale, key)}>{chrome.navigation[key]}</Link>)}</nav></div></header><main id="main-content" className="min-h-[60vh]">{children}</main><footer className="border-t border-brand-line bg-white"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6"><p className="max-w-xl text-sm leading-relaxed text-brand-subtle">{chrome.footer.description}</p><nav className="mt-5 flex flex-wrap gap-x-5 gap-y-3 text-sm text-brand-subtle" aria-label="Footer navigation">{FOOTER_KEYS.filter((key) => chrome.footer[key]).map((key) => <Link key={key} href={destination(locale, key)}>{chrome.footer[key]}</Link>)}</nav></div></footer></>;
}
