...
export function ComparisonDirectory({ locale }) {
  ...
  if (locale === 'en') return <main id="main-content" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
    ...
    <section className="mt-14 grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><article className="rounded-[2rem] border border-brand-line bg-white p-7 sm:p-9"><p className="section-kicker">How to use the pair guides</p><h2 className="display-font mt-2 text-3xl text-brand-ink">Turn a type contrast into a useful conversation</h2><ol className="mt-6 space-y-4">{['Read the shared foundation before focusing on differences.', 'Choose the context that matters now: relationship, friendship, work, or conflict.', 'Treat each pattern as a hypothesis and compare it with real behaviour.', 'Agree on one concrete adjustment instead of declaring a person compatible or incompatible.'].map((step, index) => <li key={step} className="flex gap-4"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-brand-teal text-sm font-bold text-white">{index + 1}</span><span className="pt-1 text-brand-subtle">{step}</span></li>)}</ol></article><aside className="rounded-[2rem] bg-brand-teal p-7 text-white sm:p-9"><p className="section-kicker text-brand-sand">Understand the letters</p><h2 className="display-font mt-2 text-3xl">Start with the preference pairs</h2><p className="mt-4 text-white/80">If the four-letter codes are new, learn what each preference means before interpreting a pair.</p><div className="mt-6 flex flex-col items-start gap-3"><Link href="/en/guides/mbti-letters-meaning" className="button-light">What the MBTI letters mean</Link><Link href="/en/guides/sensing-vs-intuition" className="button-dark-outline">Sensing vs Intuition</Link><Link href="/en/guides/thinking-vs-feeling" className="button-dark-outline">Thinking vs Feeling</Link></div></aside></section>
