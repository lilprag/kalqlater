...
assert.deepEqual(ASSESSMENT_LOCALES.communication, ['en', 'hi', 'fr', 'ja']);
...
for (const locale of ['en', 'hi', 'fr', 'ja']) assert.equal(isAssessmentLocaleSupported('communication', locale), true);
for (const analyzer of ['conflict', 'leadership', 'learning']) {
  ...
  assert.equal(isAssessmentLocaleSupported(analyzer, 'ja'), false);
}
